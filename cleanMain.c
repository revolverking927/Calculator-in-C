...
void appendKeyValue(keyValue table[], int* size, char key[], char value[], struct numOf** signs, double func(double, double)) {
...
double stringToNum(char* string) {
    char *endPtr;
    double num = strtod(string, &endPtr);
    if (*endPtr == '\0') {
        // printf("The number is: %.6g\n", num);
    } else {
...
void shiftSplit(char (*arr)[15], int* numSplits, double result, int index) {
...
void calcMDAS(struct numOf* sign, char (*arr)[15], int* numSplits) {
    if (sign->amountOfSymbol > 0) {
        bool operationPerformed = true;
        while (operationPerformed) {
            operationPerformed = false;
            for (int i = 1; i < *numSplits; i += 2) {
                if (arr[i][0] == sign->symbolName[0]) {
                    double num1 = stringToNum(arr[i - 1]);
                    double num2 = stringToNum(arr[i + 1]);
                    double result = sign->function(num1, num2);
                    printf("result of %.6g %c %.6g is: %.6g\n", num1, arr[i][0], num2, result);
                    shiftSplit(arr, numSplits, result, i);
                    operationPerformed = true;
                    break; // Start over after each operation
                }
            }
        }
    }
    if (sign->amountOfSymbol > 0) {
        printf("\tThe result after %s operation is ", sign->symbolName);
...
int main(void) {
    ...
    char (*splits)[15] = malloc(computedMaxSet * sizeof(*splits));
    ...
    for (int i = 0; i < maxSplits; i++) {
        ...
        if (i % 2 != 0) {
            ...
            char* cleaned = removeWhitespaces(splits[i]);
            strcpy(splits[i], cleaned);
            free(cleaned);
        }
    ...
    // MDAS order - Multiplication and Division together (left to right), then Addition, then Subtraction
    // First pass: Handle * and / from left to right
    bool operationPerformed = true;
    while (operationPerformed) {
        operationPerformed = false;
        for (int i = 1; i < maxSplits; i += 2) {
            if (splits[i][0] == '*' || splits[i][0] == '/') {
                double num1 = stringToNum(splits[i - 1]);
                double num2 = stringToNum(splits[i + 1]);
                double result;
                if (splits[i][0] == '*') {
    ...
    printf("\tThe result after multiplication/division operations is ");
    for (int i = 0; i < maxSplits; i++) {
        printf("%s", splits[i]);
    }
    printf("\n");
    ...
    // Final result with rounding to 6 decimal places
    double finalResult = stringToNum(splits[0]);
    double roundedResult = round(finalResult * 1000000.0) / 1000000.0;
    
    printf("\nResult to the problem %s = %.6g\n", input, roundedResult);
